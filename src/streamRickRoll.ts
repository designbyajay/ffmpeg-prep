...
export default async () => {
    ...
    try {
        ...
        await rmdir(streamPath, {recursive: true}); // not efficient
    ...
    const makeHls = spawn(
        ...
        [
            ...
            '-map',
            '[v1out]',
            '-c:v:0','libx264','-x264-params','nal-hrd=cbr:force-cfr=1','-b:v:0', '5M','-maxrate:v:0','5M','-minrate:v:0', '5M', '-bufsize:v:0','10M','-preset','slow','-g','48','-sc_threshold','0','-keyint_min','48',
    ...
    return new Promise<void>(resolve => {
        makeHls.stdout.on('close', () => resolve())
    })
